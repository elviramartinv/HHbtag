...
# just do eg : cd models/HHbtag_v3_par_1; ../convert_frozenGraph.py .
# will create a frozen_optimized.pb file
...
@tf.function(input_signature=[tf.TensorSpec([1, 10, 15], tf.float32, name="MyInput")], jit_compile=True)
def infer_no_batch(x):
    ...
    return tf.identity(y_b, name="MyOutput") # named MyOutput but for some reason the name does not get picked up


import numpy as np
print(infer_no_batch.get_concrete_function()(np.random.rand(1, 10, 15)))
